...
custom_packages() {
    ...
    oc_api="https://github.com/vernesong/OpenClash/releases"
    oc_file="luci-app-openclash"

    oc_file_down="$(curl -s ${amlogic_api} | grep "browser_download_url" | grep -oE "https.*${oc_file}.*.ipk" | head -n 1)"
    wget ${oc_file_down} -q -P packages
    [[ "${?}" -eq "0" ]] || error_msg "[ ${amlogic_file} ] download failed!"
    echo -e "${INFO} The [ ${oc_file} ] is downloaded successfully."
...
rebuild_firmware() {
    ...
    my_packages="\
        ...
        xz-utils ziptool zoneinfo-asia zoneinfo-core zstd ca-certificates ipset ip-full \
        libcap libcap-bin ruby ruby-yaml kmod-tun kmod-inet-diag kmod-nft-tproxy kmod-usb-net-rndis \
